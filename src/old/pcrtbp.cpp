...
pcrtbp::pcrtbp() = default;
pcrtbp::~pcrtbp() = default;
pcrtbp::pcrtbp(double mu) : mu(mu) {}
pcrtbp::pcrtbp(double mu, char option) : mu(mu), option(option) {}

pcrtbp::planar_crtbp_ode::planar_crtbp_ode(double mu) : mu(mu) {}

void pcrtbp::planar_crtbp_ode::operator()(const vec4 &x, vec4 &dxdt, double t) {
    double r1, r2;
    double xmu = x[0] + mu;
    ...
    dxdt[0] = x[2];
    dxdt[1] = x[3];
    dxdt[2] = -(1 - mu) * xmu / r1 - mu * (xmu - 1) / r2 + x[0] + 2 * x[3];
    dxdt[3] = -(1 - mu) * x[1] / r1 - mu * x[1] / r2 + x[1] - 2 * x[2];
}

double pcrtbp::jacobi_inte(const vec4 &x) {
    double xmu = x[0] + mu;
    double y2 = pow(x[1], 2);
    double r1, r2;
    r1 = sqrt(pow(xmu, 2) + y2);
    r2 = sqrt(pow(xmu - 1, 2) + y2);
    return x[0] * x[0] + y2 - x[2] * x[2] - x[3] * x[3] + 2 * (1 - mu) / r1 +
           2 * mu / r2;
}

double pcrtbp::jacobi_inte_ele(const vec4 &elements) {
    double a = elements[0];
    double e = elements[1];
    double result = 0.;
    if (option == 'p') {
...
void pcrtbp::integ_output(vec4 x, const double endtime, const double dt) {
...
void pcrtbp::write_keypoint(const vec4 &x, const double t) {
    outputfile4 << t << '\t';
    auto y = rot2init(x, t);
...
void pcrtbp::write_detail(const vec4 &x, const double t) {
    outputfile1 << t << '\t';
    outputfile2 << t << '\t';
    outputfile3 << t << '\t';
    auto y = rot2init(x, t);
...
vec4 pcrtbp::rot2init(vec4 x, const double t) {
    double c = cos(t);
    double s = sin(t);
    x[0] += mu;
...
vec4 pcrtbp::init2rot(vec4 x, const double t) {
...
double pcrtbp::rot2r(vec4 x) { return x[0] * x[2] + x[1] * x[3]; }

vec4 pcrtbp::vector2elements(const vec4 &x) {
    // input: {{x,y,vx,vy}}
    // output: {{a,e,M,omega}}
    ...
    // cout << vr << '\t' << vv << endl;
    double e = sqrt(pow(vv * h / (1 - mu) - 1, 2) + pow(vr * h / (1 - mu), 2));
...
double pcrtbp::true2mean(double theta, double e) {
    double E = 2 * atan(sqrt((1 - e) / (1 + e)) * tan(theta / 2));
    return E - e * sin(E);
}

vec4 pcrtbp::elements2vector(const vec4 &elements) {
...
void pcrtbp::print_vec(const vec4 &x) {
    for (auto ele : x) {
        cout << ele << '\t';
    }
    cout << endl;
...
void pcrtbp::cal_init_vec() {
    auto x = elements2vector(elements);
    vector = init2rot(x, 0);
}

double pcrtbp::F1_func(const vec4 &x, const double t) {
...
double pcrtbp::average(double a, double e, int limit) {
    // outputfile1.open("../assets/rot.txt");
    // outputfile2.open("../assets/init.txt");
    // outputfile3.open("../assets/elements.txt");
    elements = {{a, e, 0, 0}};
...
double pcrtbp::delta21(double a) {
...
double pcrtbp::Hamiltion21(double delta, double Phi, double phi) {
    return delta * Phi + Phi * Phi - 2 * sqrt(2 * Phi) * cos(phi);
}

double pcrtbp::deltax1x2(double x1, double x2) {
