...
class resonance {
  public:
    resonance() = default;
    ~resonance() = default;
    resonance(size_t k, size_t kj, double N, double Sz, double Min, double Max,
              size_t numY, size_t num)
        : k(k), kj(kj), N(N), Sz(Sz), Min(Min), Max(Max), numY(numY),
          num(num){};
    resonance(size_t k, size_t kj, double N, double a, int phi, double Max,
              size_t numY, size_t num)
        : k(k), kj(kj), N(N), a(a), phi((double)phi), phi_c((double)phi),
